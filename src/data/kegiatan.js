export const kegiatanData = [
  {
    id: 1,
    title: "Apel Pagi Rutin di Pendopo Kecamatan",
    date: "12 Januari 2026",
    image: "/images/kegiatan/apel-pagi.jpg",
    description: "Kegiatan apel pagi rutin yang diikuti oleh seluruh staf kecamatan untuk meningkatkan kedisiplinan dan koordinasi pelayanan.",
    content: `
      <p><strong>PAKONG</strong> – Kecamatan Pakong kembali melaksanakan rutinitas Apel Pagi setiap hari Senin. Kegiatan ini berlangsung khidmat di halaman Pendopo Kecamatan Pakong pada pukul 07.30 WIB, diikuti oleh seluruh Aparatur Sipil Negara (ASN) dan tenaga honorer di lingkungan pemerintahan kecamatan.</p>

      <p>Camat Pakong, dalam amanatnya, menekankan beberapa poin penting terkait peningkatan kinerja dan pelayanan publik:</p>

      <h3>1. Kedisiplinan Pegawai</h3>
      <p>Kedisiplinan bukan hanya soal kehadiran tepat waktu, tetapi juga tentang tanggung jawab terhadap tugas pokok dan fungsi masing-masing. Seluruh staf diharapkan dapat menjadi contoh yang baik bagi masyarakat.</p>

      <h3>2. Percepatan Pelayanan Administrasi</h3>
      <p>Mengingat tingginya permintaan layanan administrasi kependudukan di awal tahun, seluruh petugas loket diminta untuk bekerja lebih cekatan dan ramah. "Kita harus pastikan warga tidak menunggu terlalu lama," tegas Bapak Camat.</p>

      <h3>3. Kebersihan Lingkungan Kantor</h3>
      <p>Selain kinerja, kebersihan lingkungan kerja juga menjadi sorotan. Lingkungan yang bersih akan menciptakan suasana kerja yang nyaman dan memberikan kesan positif bagi warga yang datang mengurus keperluan.</p>

      <p>Apel pagi ditutup dengan doa bersama demi kelancaran tugas-tugas pelayanan masyarakat sepanjang minggu ini.</p>
    `
  },
  {
    id: 2,
    title: "Rapat Koordinasi Perangkat Desa",
    date: "10 Januari 2026",
    image: "/images/kegiatan/rapat-desa.jpg",
    description: "Musyawarah perencanaan pembangunan desa yang melibatkan seluruh kepada desa se-Kecamatan Pakong.",
    content: `
      <p><strong>PAKONG</strong> – Mengawali tahun anggaran 2026, Kecamatan Pakong menggelar Rapat Koordinasi (Rakor) bersama seluruh Kepala Desa dan perangkat desa se-Kecamatan Pakong. Bertempat di Aula Kecamatan, rapat ini bertujuan untuk menyamakan persepsi dan langkah strategis dalam pembangunan desa.</p>

      <p>Fokus utama pembahasan dalam rapat kali ini meliputi:</p>
      
      <ul>
        <li><strong>Evaluasi APBDes Tahun Lalu:</strong> Pembahasan mengenai capaian dan kendala yang dihadapi desa dalam merealisasikan anggaran tahun sebelumnya.</li>
        <li><strong>Prioritas Dana Desa 2026:</strong> Sosialisasi mengenai peraturan terbaru penggunaan Dana Desa, yang tahun ini diprioritaskan untuk penanganan kemiskinan ekstrem, ketahanan pangan, dan penurunan stunting.</li>
        <li><strong>Program Digitalisasi Desa:</strong> Dorongan bagi desa-desa untuk mulai beralih ke pelayanan berbasis digital guna memudahkan administrasi warga.</li>
      </ul>

      <p>Sekretaris Kecamatan (Sekcam) menambahkan bahwa sinergitas antara pemerintah desa dan kecamatan adalah kunci suksesnya pembangunan wilayah. "Tidak ada desa yang bisa maju sendirian, kita harus saling mendukung dan berkolaborasi," ujarnya.</p>

      <p>Kegiatan diakhiri dengan sesi tanya jawab interaktif, di mana para Kepala Desa menyampaikan aspirasi dan kendala teknis di lapangan untuk dicarikan solusinya bersama.</p>
    `
  },
  {
    id: 3,
    title: "Musrenbang Kecamatan Tahun 2026",
    date: "28 Januari 2026",
    image: "/images/kegiatan/musrenbang.jpg",
    description: "Forum musyawarah perencanaan pembangunan tingkat kecamatan untuk menyepakati RKP Desa tahun rencana.",
    content: `
      <p><strong>PAKONG</strong> – Kecamatan Pakong telah melaksanakan Musyawarah Perencanaan Pembangunan (Musrenbang) tingkat kecamatan untuk penyusunan Rencana Kerja Pemerintah (RKP) Desa tahun 2027. Kegiatan ini dihadiri oleh berbagai elemen masyarakat, perwakilan desa, dan instansi terkait.</p>

      <p>Dalam forum ini, dibahas berbagai usulan prioritas pembangunan yang mencakup:</p>
      <ul>
        <li>Peningkatan infrastruktur jalan desa dan jembatan.</li>
        <li>Program pemberdayaan ekonomi masyarakat melalui UMKM.</li>
        <li>Peningkatan layanan kesehatan dan pendidikan.</li>
      </ul>

      <p>Camat Pakong berharap hasil Musrenbang ini dapat mengakomodasi aspirasi masyarakat dan terealisasi dengan baik pada tahun anggaran mendatang.</p>
    `
  }
];
